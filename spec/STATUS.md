...
- 2026-09-24 · FINAL · Regressão final serial (T00-T22, operario+tester = 46 verificações): 45 ok e T08 tester falhando por AMBIENTE. Diagnóstico do Cinzel: localhost→::1 e o libpq perde ~2 s por conexão com as portas só em 127.0.0.1, bloqueando o event loop dos testes. Correção: verify.mjs exporta DATABASE_URL/REDIS_URL com 127.0.0.1 (+ helpers/pg.ts, .env.example, INFRA.md). T08 22/22, T00 e T16 op reverificados. PROJETO CONCLUÍDO: T00-T22 ACCEPTED (T20 automático recusado e substituído pela versão manual). · Orquestrador
- 2026-09-24 · harness · Causa raiz dos page.goto pendurados nas suítes de dashboard: o servidor do harness (T12/shared.ts) fazia writeHead antes de ler o arquivo, e um rebuild concorrente do dist gerava ERR_HTTP_HEADERS_SENT com a resposta pendurada. Corrigido pela Íris: leitura antes do cabeçalho, lock de build e cópia privada do dist por servidor. · Orquestrador
